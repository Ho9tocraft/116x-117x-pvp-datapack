...
execute as @a[scores={ship-num=6,skill2-ct=0,FocusPoint=80..},predicate=ship-based-pvp-datapacks:soothing-sunlight,team=Red] at @s as @e[team=Red,distance=..8] at @s run effect give @s instant_health 1 5 false
execute as @a[scores={ship-num=6,skill2-ct=0,FocusPoint=80..},predicate=ship-based-pvp-datapacks:soothing-sunlight,team=Blue] at @s as @e[team=Blue,distance=..8] at @s run effect give @s instant_health 1 5 false
#1.5: 演出パーティクル
execute as @a[scores={ship-num=6,skill2-ct=0,FocusPoint=80..},predicate=ship-based-pvp-datapacks:soothing-sunlight] at @s run particle dust 0.953 0.596 0.000 1 ~ ~ ~ 8.0 0.2 8.0 1 40 normal @a
#2: 使用者は追加で回復（範囲はない）
