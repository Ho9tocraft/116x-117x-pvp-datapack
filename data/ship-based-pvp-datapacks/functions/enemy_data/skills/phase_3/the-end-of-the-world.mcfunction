#ジ・エンド・オブ・ザ・ワールド
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run tellraw @a [{"text": "死した神稚児","color": "dark_red"},{"text": ": ","color": "white"},{"text": "神の怒りに砕け散れぇぇぇェェェエエエッ！！"}]
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run tellraw @a {"text": "死した神稚児は「火継ぎの終わりを看取るもの」の構え。","color": "green"}
...
#CT処理
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run scoreboard players set @s skill3-ct 120
