...
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run tellraw @a {"text": "死した神稚児は「冠位指定／人理保障天球」の構え。","color": "green"}
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run tellraw @a {"text": "死した神稚児の「嵐の落雷」","color": "green"}
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run effect give @s resistance 10 4 true
...
execute as @e[type=skeleton,tag=Yuzuki_HandHeld,scores={skill3-ct=0}] at @s run schedule function ship-based-pvp-datapacks:enemy_data/skills/phase_3/animsphere-pose 7s
